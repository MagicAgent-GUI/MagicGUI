...
def process_step_data(step_data, evaluator, save_dir):
    ...
    try:
        ...
        if not pred:
            logger.error(f"data path {cur_save_path} ,action predict result is None")
            return None
...
def evaluate(args):
    ...
    output_abs_dir = os.path.abspath(args.output_dir)
    convert2aitz(input_abs_path, output_abs_dir, max_workers=16)
    ...
    try:
        ...
        fd = os.open(results_save_file, os.O_WRONLY | os.O_CREAT, 0o600)  
    ...
    try:
        ...
        fd_sum = os.open(summary_save_file, os.O_WRONLY | os.O_CREAT, 0o600)  
        with os.fdopen(fd_sum, 'w') as f:
