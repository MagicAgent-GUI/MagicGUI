...
from .utils import annotate_and_save_image
...
_TAP_DISTANCE_THRESHOLD_AC = 0.14  # for android control, align with qwen's code.
...
def _get_direction(point1, point2):
    try:
        ...
        if not x1 or not x2  or not y1 or not y2:
            raise ValueError(f"Error point1 {point1} ,  point2 {point2} ")
...
class HonorActionEvaluator(object):
    ...
    def __call__(self, gt, pred, annotate_image=False):
        ...
        if type_match and pd_action_type == "scroll":
            # if pd_action_direction == "down" and gt_action_direction == "up":
            #     exact_match = True
            # if pd_action_direction == "up" and gt_action_direction == "down":
            #     exact_match = True
            # if pd_action_direction == "left" and gt_action_direction == "right":
            #     exact_match = True
            # if pd_action_direction == "right" and gt_action_direction == "left":
            #     exact_match = True
        ...
        if type_match and pd_action_type == "type":
            ...
            exact_match = calculate_f1_score(pd_text_norm, gt_text_norm) >= 0.5
        ...
        if type_match and gt_action_type == "stop":
            exact_match = True

        if type_match and pd_action_type == "wait":
            exact_match = True

        if gt_action_type == "click" and pd_action_type == "type":
            type_match = True
            exact_match = True

        if gt_action_type == "type" and pd_action_type == "click":
            type_match = True
            exact_match = True

        if gt_action_type == "press":
            if pd_action_type == "click" and pd_action_yx["y"] <= 0.15 and pd_action_yx["x"] <= 0.15:
                type_match = True
                exact_match = True
    ...
    def compute_atomic_metrics(step_results):
        ...
        scores = {
            ...
            for metric_key in ['total', 'CLICK', 'LONG_POINT', 'SCROLL', 'PRESS', 'STOP', 'WAIT', 'TYPE']
        }

        click_num = recorder['CLICK']['count']
        click_type_correct = recorder['CLICK']['type_match']
        click_exact_correct = recorder['CLICK']['exact_match']


        scroll_num = recorder['SCROLL']['count']
        scroll_type_correct = recorder['SCROLL']['type_match']
        scroll_exact_correct = recorder['SCROLL']['exact_match']


        type_num = recorder['TYPE']['count']
        type_type_correct = recorder['TYPE']['type_match']
        type_exact_correct = recorder['TYPE']['exact_match']

        back_num = recorder['PRESS']['count']
        back_type_correct = recorder['PRESS']['type_match']
        back_exact_correct = recorder['PRESS']['exact_match']

        finish_num = recorder['STOP']['count']
        finish_type_correct = recorder['STOP']['type_match']
        finish_exact_correct = recorder['STOP']['exact_match']

        CLICK_SCROLL_TYPE_num = click_num + scroll_num + type_num
        CLICK_SCROLL_TYPE_type_acc = float((click_type_correct + scroll_type_correct + type_type_correct) / CLICK_SCROLL_TYPE_num)
        CLICK_SCROLL_TYPE_exact_acc = float((click_exact_correct + scroll_exact_correct + type_exact_correct) / CLICK_SCROLL_TYPE_num)

        scores["CLICK_SCROLL_TYPE"]= {"Total": CLICK_SCROLL_TYPE_num, "type_acc": CLICK_SCROLL_TYPE_type_acc, "exact_acc": CLICK_SCROLL_TYPE_exact_acc}

        CLICK_SCROLL_TYPE_BACK_num = click_num + scroll_num + type_num + back_num
        CLICK_SCROLL_TYPE_BACK_type_acc = float((click_type_correct + scroll_type_correct + type_type_correct + back_type_correct) / CLICK_SCROLL_TYPE_BACK_num)
        CLICK_SCROLL_TYPE_BACK_exact_acc = float((click_exact_correct + scroll_exact_correct + type_exact_correct + back_exact_correct) / CLICK_SCROLL_TYPE_BACK_num)

        scores["CLICK_SCROLL_TYPE_BACK"]= {"Total": CLICK_SCROLL_TYPE_BACK_num, "type_acc": CLICK_SCROLL_TYPE_BACK_type_acc, "exact_acc": CLICK_SCROLL_TYPE_BACK_exact_acc}

        CLICK_SCROLL_TYPE_BACK_FINISH_num = click_num + scroll_num + type_num + back_num + finish_num
        CLICK_SCROLL_TYPE_BACK_FINISH_type_acc = float((click_type_correct + scroll_type_correct + type_type_correct + back_type_correct + finish_type_correct) / CLICK_SCROLL_TYPE_BACK_FINISH_num)
        CLICK_SCROLL_TYPE_BACK_FINISH_exact_acc = float((click_exact_correct + scroll_exact_correct + type_exact_correct + back_exact_correct + finish_exact_correct) / CLICK_SCROLL_TYPE_BACK_FINISH_num)

        scores["CLICK_SCROLL_TYPE_BACK_FINISH"]= {"Total": CLICK_SCROLL_TYPE_BACK_FINISH_num, "type_acc": CLICK_SCROLL_TYPE_BACK_FINISH_type_acc, "exact_acc": CLICK_SCROLL_TYPE_BACK_FINISH_exact_acc}
