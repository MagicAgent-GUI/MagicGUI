...
def decode_img_omni(tup):
    ...
    if len(images) != len(paths):
        raise ValueError(f'Error images size {len(images)} not match len paths {len(paths)}')
...
def localize_df(data, dname, nproc=32):
    if 'image' not in data:
        raise ValueError('Error data type , image not in it ')
        ...
        for i in indices_str:
            if len(image_map[i]) <= 64:
                ...
                if idx not in image_map or len(image_map[idx]) <= 64:
                    raise ValueError(f'Error image_map , key {idx} not in it , or value len less than 64')
...
#         fd = os.open(data, os.O_WRONLY | os.O_CREAT, 0o600) 
...
def dump(data, f, **kwargs):
    def dump_pkl(data, pth, **kwargs):
        fd = os.open(pth, os.O_WRONLY | os.O_CREAT, 0o600) 
    ...
    def dump_json(data, pth, **kwargs):
        ...
        with os.fdopen(fd, 'w', encoding='utf-8') as fout:
            json.dump(data, fout, indent=4, ensure_ascii=False, cls=NumpyEncoder)

    def dump_jsonl(data, pth, **kwargs):
        lines = [json.dumps(x, ensure_ascii=False, cls=NumpyEncoder) for x in data]
        fd = os.open(pth, os.O_WRONLY | os.O_CREAT, 0o600) 
        with os.fdopen(fd, 'w', encoding='utf8') as fout:
            fout.write('\n'.join(lines))

    def dump_xlsx(data, pth, **kwargs):
        data.to_excel(pth, index=False, engine='xlsxwriter')

    def dump_csv(data, pth, quoting=csv.QUOTE_ALL):
        data.to_csv(pth, index=False, encoding='utf-8', quoting=quoting)

    def dump_tsv(data, pth, quoting=csv.QUOTE_ALL):
        data.to_csv(pth, sep='\t', index=False, encoding='utf-8', quoting=quoting)
...
def load(f, fmt=None):
    def load_pkl(pth):
        try:
            fd = os.open(pth, os.O_RDWR | os.O_CREAT, 0o600) 
            with os.fdopen(fd, 'rb') as file:
                return pickle.load(file)
        except:
            raise ValueError(f'Error pickle load , model path {pth}')
    def load_json(pth):
...
def ls(dirname='.', match=[], mode='all', level=1):
    if isinstance(level, str):
        if '+' not in level:
            raise ValueError(f"Error level {level}, '+' not in it")
        level = int(level[:-1])
        res = []
    ...
    if mode not in ['all', 'dir', 'file']:
        raise ValueError(f"Error mode {mode}, should be in ['all', 'dir', 'file']")
    if  level < 1 or not isinstance(level, int):
        raise ValueError(f"Error level {level}, less than 1 or not int type")
    if level == 1:
...
def mwlines(lines, fname):
    fd = os.open(fname, os.O_RDWR | os.O_CREAT, 0o600) 
...
def parse_file(s):
    if osp.exists(s) and s != '.':
        if not osp.isfile(s):
            raise ValueError(f"path {s} is not a file")
