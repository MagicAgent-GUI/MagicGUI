...
def h2r(value):
    if value[0] == '#':
        value = value[1:]
    if len(value) != 6:
        raise ValueError(f'Error value {value} not equal to 6' )
...
def listinstr(lst, s):
    if not isinstance(lst, list):
        raise ValueError(f'Error data type {type(lst)} not match "list"')
    for item in lst:
        if item in s:
            return True
    return False
...
def timestr(granularity='second'):
    ...
    if granularity not in ['second', 'minute', 'hour', 'day']:
        raise ValueError(f"Error time format {granularity}, should be in ['second', 'minute', 'hour', 'day']")
...
def githash(fallback='unknown', digits=8):
    ...
    try:
        ...
        return fallback
    try:
        out = _minimal_ext_cmd(['git', 'rev-parse', 'HEAD'], cwd=utils.__path__[0])
...
def load_env():
    ...
    try:
        ...
        logging.error('Failed to import environment variables from .env file. ')
        raise ValueError('Failed to import environment variables from .env file. ')
    pth = osp.realpath(utils.__path__[0])
